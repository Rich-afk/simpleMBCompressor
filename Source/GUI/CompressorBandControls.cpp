...
void CompressorBandControls::toggleAllBands(bool shouldBeBypassed)
{
    ...
    for (auto *band : bands)
    {
        band->setColour(juce::TextButton::ColourIds::buttonOnColourId,
                        shouldBeBypassed ? bypassButton.findColour(juce::TextButton::ColourIds::buttonOnColourId) : ColorScheme::getSliderBorderColor());

        band->setColour(juce::TextButton::ColourIds::buttonColourId,
                        shouldBeBypassed ? bypassButton.findColour(juce::TextButton::ColourIds::buttonOnColourId) : juce::Colours::black);
        band->repaint();
    }
}
...
void CompressorBandControls::updateBandSelectButtonStates()
{
    ...
    for (size_t i = 0; i < paramsToCheck.size(); ++i)
    {
        ...
        auto *bandButton = (i == 0) ? &lowBand : (i == 1) ? &midBand
                                                          : &highBand;
